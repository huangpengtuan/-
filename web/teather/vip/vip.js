	...
	.controller('vipCtl',[
		...
		function($scope){
			...
			var resetvip=function(){
		    	$.ajax({
		    		...
		    		success:function(data){	
						...
						$scope.teacherVip=data.teacherVip;
						$scope.sex=data.sex;
						$scope.avatar=data.avatar;
						$scope.vip_end_time=data.teacherVipEndTime;	
						$scope.$apply();
			...
			$scope.submitpay=function(Totalamount,balance){			
				if($scope.paytype == 1){
					window.location.href="/c/pay/openVipByWechatPay?type="+$scope.viptype+"&no="+$scope.viptime+"&identity=2"; 
